...
export function Navbar({ postInfo, children }: NavbarProps) {
  ...
  const linkListPart = useMemo(() => {
    if (isDesktop) {
      return <NavLinkList pathname={pathname} />;
    }
    if (shouldShow) {
      return <div></div>;
    }
    return <NavLinkList pathname={pathname} />;
  }, [isDesktop, shouldShow, pathname]);
  ...
  const logoPart = useMemo(() => {
    ...
    if (shouldShow) {
      return <NavLinksDrawer pathname={pathname} />;
    }
    return <Logo>{children}</Logo>;
  }, [isDesktop, shouldShow, children, pathname]);
